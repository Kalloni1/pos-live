...
st.write("This calculator uses verified purchase data to estimate customer satisfaction based on product behavior, not just written reviews.")

sales = st.number_input("Total Verified Sales", min_value=0, value=10000)
returns = st.number_input("Verified Returns", min_value=0, value=500)
...
if st.button("Calculate P.O.S. Rating"):
    ...
    st.success(f"Estimated P.O.S. Rating (Probability of Satisfaction): {pos} stars")
